...
export class NextEncounterPhase extends EncounterPhase {
  ...
  doEncounter(): void {
    ...
    for (const pokemon of this.scene.getParty()) {
      if (pokemon) {
        // TODO: Build option to toggle this on/off
        // pokemon.resetBattleData();
      }
    }

    // Check if player has map modifier
    const hasMap = this.scene.findModifier(m => m instanceof MapModifier);
    if (hasMap) {
      // Give the player the option to select the next biome
      this.scene.unshiftPhase(new SelectBiomePhase(this.scene));
    }
  ...
  start() {
    ...
    const chooseNextBiome = () => {
      ...
      if (biomes.length > 1 && this.scene.findModifier(m => m instanceof MapModifier)) {
        ...
        this.scene.executeWithSeedOffset(() => {
          const standardChoices = (!Array.isArray(biomeLinks[currentBiome])
            ? [biomeLinks[currentBiome] as Biome]
            : biomeLinks[currentBiome] as (Biome | [Biome, integer])[])
            .filter((b, i) => !Array.isArray(b) || !Utils.randSeedInt(b[1]))
            .map(b => Array.isArray(b) ? b[0] : b);
          const randomBiomeCount = 3;
          const allBiomeKeys = Object.keys(biomeLinks);
          const generateRandomBiome = () => {
            const biomeKey = allBiomeKeys[Math.floor(Math.random() * allBiomeKeys.length)];
            const biomeLink = biomeLinks[biomeKey] as Biome;
            const randomBiome = Array.isArray(biomeLink) ? biomeLink[Math.floor(Math.random() * (biomeLink.length + 1))] : biomeLink;
            if (Array.isArray(randomBiome)) {
              return randomBiome[Math.floor(Math.random() * (randomBiome.length + 1))];
            }
            if (randomBiome !== undefined && !standardChoices.includes(randomBiome)) {
              return randomBiome;
            } else {
              return generateRandomBiome();
            }
          };
          const randomBiomes = Array.from({ length: randomBiomeCount }, () => {
            return generateRandomBiome();
          });
          biomeChoices = [...standardChoices, ...randomBiomes];
    ...
    // if (this.scene.gameMode.isClassic && hasMap) {
    //   chooseNextBiome();
    // }
...
export class CommandPhase extends FieldPhase {
  ...
  handleCommand(command: Command, cursor: integer, ...args: any[]): boolean {
    ...
    case Command.BALL:
      if (this.scene.arena.biomeType === Biome.END && (!this.scene.gameMode.isClassic || (this.scene.getEnemyField().filter(p => p.isActive(true)).some(p => !p.scene.gameData.dexData[p.species.speciesId].caughtAttr) && this.scene.gameData.getStarterCount(d => !!d.caughtAttr) < Object.keys(speciesStarters).length - 1))) {
      ...
      } else if (this.scene.currentBattle.battleType === BattleType.TRAINER && !this.trainerTheft) {
        this.scene.ui.setMode(Mode.COMMAND, this.fieldIndex);
        this.scene.ui.setMode(Mode.MESSAGE);
        ...
        if (targets.length > 1) {
          ...
          if ((targetPokemon.isBoss() && targetPokemon.bossSegmentIndex >= 1 && !targetPokemon.hasAbility(Abilities.WONDER_GUARD, false, true)) && (!PokeballType.ROGUE_BALL || !PokeballType.MASTER_BALL)) {
            this.scene.ui.setMode(Mode.COMMAND, this.fieldIndex);
            this.scene.ui.setMode(Mode.MESSAGE);
    ...
    case Command.RUN:
      ...
      } else if (!isSwitch && this.scene.currentBattle.battleType === BattleType.TRAINER && !this.trainerTheft) {
        this.scene.ui.setMode(Mode.COMMAND, this.fieldIndex);
        this.scene.ui.setMode(Mode.MESSAGE);
...
export class SelectModifierPhase extends BattlePhase {
  ...
  start() {
    ...
    const modifierCount = new Utils.IntegerHolder(7);
    if (this.isPlayer()) {
    ...
    const modifierSelectCallback = (rowCursor: integer, cursor: integer) => {
      ...
      case 0:
        if (!cursor) {
          ...
          const rerollCost = 500;
          const additionalFunds = rerollCost * (this.rerollCount + 1);
          this.scene.money += additionalFunds;
